...
export async function getDataDrivenResourceAnimationControllerIds(_document: vscode.TextDocument, _schema: MinecraftJsonSchema): Promise<string[]> {
...
export async function getResourceAnimationControllerIds(_document: vscode.TextDocument, _schema: MinecraftJsonSchema): Promise<string[]> {
    ...
    const vanillaUris = await getStableDataManager()?.getFiles("resource_pack/animation_controllers/<all>.json") ?? [];
    const projectUris = await getCurrentProject()?.fileResolver.getDataDrivenFilesFromProject("resource_pack/animation_controllers/<all>.json") ?? [];
